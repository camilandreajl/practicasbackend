...
export const getSignedUrlFromS3 = async (bucketName: string, path: string): Promise<string> => {
...
export const getSignedUrlForUpload = async (path: string): Promise<string> => {
  ...
  return getSignedUrl(s3Client, command, { expiresIn: WRITE_EXPIRATION_SECONDS });
};
